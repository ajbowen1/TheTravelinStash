package edu.asu.bsse.ajbowen1.thetravelinstash;

import android.content.Intent;
import android.support.v7.app.AppCompatActivity;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;
/**
 * Copyright © 2017 Avery J. Bowen,
 * <p/>
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p/>
 * http://www.apache.org/licenses/LICENSE-2.0
 * <p/>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 * <p/>
 * Purpose: An app to maintain a user's yarn stash
 *
 * @author Avery J. Bowen ajbowen1@asu.edu
 * @version April 28, 2017
 */
public class AddActivitySuccess extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        Button mainMenu = (Button)findViewById(R.id.mainButton);
        Intent i = getIntent();
        if(i.getStringExtra("success").equals("add")){
            setContentView(R.layout.activity_add_success);
        }
        if(i.getStringExtra("success").equals("modify")){
            setContentView(R.layout.activity_mod_success);
        }
        if(i.getStringExtra("success").equals("remove")){
            setContentView(R.layout.activity_remove_success);
        }
    }

    public void mainClicked(View v) {
        Intent mainIntent = new Intent(this, MainActivity.class);
        startActivity(mainIntent);
    }
}
